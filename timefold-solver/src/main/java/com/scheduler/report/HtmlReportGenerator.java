...
public class HtmlReportGenerator {
    ...
    private static String buildScoreBanner(ScheduleSolution solution) {
        ...
        long assignedCount = solution.getShiftSlots().stream()
            ...
            .count();
        // Count closing slots with staff assigned
        long closingAssigned = solution.getClosingSlots().stream()
            .filter(s -> s.getStaff() != null)
            .count();
    ...
    private static String buildWorkloadTable(ScheduleSolution solution) {
        ...
        // Count closing from ShiftSlot closing roles
        for (ShiftSlot closingSlot : solution.getClosingSlots()) {
            if (closingSlot.getStaff() == null) continue;
            Map<String, Integer> stats = staffStats.get(closingSlot.getStaff());
            if (stats != null) {
    ...
    private static String buildChartScripts(ScheduleSolution solution, LocalDate startDate, LocalDate endDate) {
        ...
        // Closing data (from ShiftSlot closing roles)
        Map<String, int[]> closingByStaff = new LinkedHashMap<>(); // [1R count, 2F count]
        for (ShiftSlot closingSlot : solution.getClosingSlots()) {
            ClosingRole role = closingSlot.getClosingRole();
            if (role != null && closingSlot.getStaff() != null) {
                String name = closingSlot.getStaff().getFullName();
    ...
    private static String buildCalendarGrid(ScheduleSolution solution,
        ...
        // Build closing map: staff+location+date -> ClosingRole (show badge on slots at that location)
        Map<String, ClosingRole> closingMap = new HashMap<>();
        for (ShiftSlot closingSlot : solution.getClosingSlots()) {
            if (closingSlot.getStaff() != null) {
                String key = closingSlot.getStaff().getId() + "|" + closingSlot.getLocationId() + "|" + closingSlot.getDate();
                closingMap.put(key, closingSlot.getClosingRole());
            }
        }
        ...
        for (Staff staff : sortedStaff) {
            ...
            // Group slots by date, separating full-day slots from half-day slots
            Map<LocalDate, List<ShiftSlot>> slotsByDate = byStaff.get(staff).stream()
                .collect(Collectors.groupingBy(ShiftSlot::getDate));

            for (LocalDate date : dates) {
                grid.append("<td class=\"day-cell\">\n");

                List<ShiftSlot> slotsOnDate = slotsByDate.getOrDefault(date, List.of());

                // Check for full-day slot (fullDaySlot=true or periodId=0)
                ShiftSlot fullDay = slotsOnDate.stream()
                    .filter(s -> s.isFullDaySlot() || s.getPeriodId() == 0)
                    .findFirst().orElse(null);

                if (fullDay != null) {
                    // Full-day slot spans both AM and PM - show as single block
                    grid.append(formatSlot(fullDay, "fullday", closingMap));
                } else {
                    // Separate AM and PM slots
                    ShiftSlot am = slotsOnDate.stream()
                        .filter(s -> s.getPeriodId() == 1)
                        .findFirst().orElse(null);
                    grid.append(formatSlot(am, "am", closingMap));

                    ShiftSlot pm = slotsOnDate.stream()
    ...
    private static String buildClosingSection(ScheduleSolution solution) {
        ...
        // Get closing slots (ShiftSlots with closing role)
        List<ShiftSlot> closingSlots = solution.getClosingSlots();
        if (closingSlots.isEmpty()) return "";

        sb.append("<div class=\"section\">\n");
        sb.append("<h2>🔐 Responsabilités Closing</h2>\n");

        Map<LocalDate, List<ShiftSlot>> byDate = closingSlots.stream()
            .sorted(Comparator.comparing(ShiftSlot::getDate)
                .thenComparing(s -> s.getLocationName() != null ? s.getLocationName() : "")
                .thenComparing(s -> s.getClosingRole() != null ? s.getClosingRole().ordinal() : 99))
            .collect(Collectors.groupingBy(ShiftSlot::getDate, LinkedHashMap::new, Collectors.toList()));

        sb.append("<table class=\"closing-table\">\n");
        sb.append("<tr><th>Date</th><th>Location</th><th>Rôle</th><th>Staff Assigné</th></tr>\n");

        for (Map.Entry<LocalDate, List<ShiftSlot>> entry : byDate.entrySet()) {
            LocalDate date = entry.getKey();
            for (ShiftSlot slot : entry.getValue()) {
                String statusClass = slot.getStaff() != null ? "assigned" : "unassigned";
                String staffName = slot.getStaff() != null ? slot.getStaff().getFullName() : "NON ASSIGNE";
                ...
                sb.append("<td>").append(escapeHtml(slot.getLocationName())).append("</td>\n");
    ...
    private static String getCSS() {
        return """
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .slot.fullday { border-left: 3px solid #9c27b0; min-height: 48px; display: flex; align-items: center; }
